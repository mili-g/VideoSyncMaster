...
class AsrRuntimeConfig:
    ...
    local_asr_max_inference_batch_size: int = 32
    ...
    def from_sources(
        ...
        return cls(
            asr_model_profile=normalize_asr_model_profile(
                service,
                _pick_str(primary, fallback, "asr_model_profile", default="")
            ),
            faster_whisper_vad_filter=_pick_bool(primary, fallback, "faster_whisper_vad_filter", default=True),
            faster_whisper_vad_threshold=_pick_float(primary, fallback, "faster_whisper_vad_threshold", default=0.4),
            funasr_batch_size_s=max(1, _pick_int(primary, fallback, "funasr_batch_size_s", default=300)),
            funasr_merge_vad=_pick_bool(primary, fallback, "funasr_merge_vad", default=True),
            local_asr_device=_pick_str(primary, fallback, "local_asr_device", fallback_keys=("qwen_device",), default="auto") or "auto",
            local_asr_max_inference_batch_size=max(1, _pick_int(primary, fallback, "local_asr_max_inference_batch_size", fallback_keys=("qwen_max_inference_batch_size",), default=32)),
            local_asr_max_new_tokens=max(32, _pick_int(primary, fallback, "local_asr_max_new_tokens", fallback_keys=("qwen_max_new_tokens",), default=256)),
        )

    def to_runner_kwargs(self) -> dict[str, Any]:
        return {
            "model_profile": self.asr_model_profile,
            "faster_whisper_vad_filter": self.faster_whisper_vad_filter,
            "faster_whisper_vad_threshold": self.faster_whisper_vad_threshold,
            "funasr_batch_size_s": self.funasr_batch_size_s,
            "funasr_merge_vad": self.funasr_merge_vad,
            "local_asr_device": self.local_asr_device,
            "local_asr_max_inference_batch_size": self.local_asr_max_inference_batch_size,
            "local_asr_max_new_tokens": self.local_asr_max_new_tokens,
            "qwen_device": self.local_asr_device,
            "qwen_max_inference_batch_size": self.local_asr_max_inference_batch_size,
            "qwen_max_new_tokens": self.local_asr_max_new_tokens,
        }
...
class TranslationRuntimeConfig:
    ...
    def from_sources(cls, primary: dict[str, Any] | None = None, fallback: dict[str, Any] | None = None) -> "TranslationRuntimeConfig":
        ...
        return cls(
            ...
            enable_local_llm=_pick_bool(primary, fallback, "enable_local_llm", fallback_keys=("enable_local_llm_split",), default=False),
        )
...
class TtsRuntimeConfig:
    tts_model_profile: str = "standard"
    ...
    def from_sources(cls, primary: dict[str, Any] | None = None, fallback: dict[str, Any] | None = None) -> "TtsRuntimeConfig":
        ...
        tts_service_name = _pick_str(primary, fallback, "tts_service", default="indextts")
        tts_model_profile = normalize_tts_model_profile(
            tts_service_name,
            _pick_str(primary, fallback, "tts_model_profile", default="")
        )
        qwen_model_size = _pick_str(primary, fallback, "qwen_model_size", default="1.7B")
        if tts_service_name == "qwen":
            if tts_model_profile == "fast":
    ...
    def to_runner_kwargs(self) -> dict[str, Any]:
        return {
            "tts_model_profile": self.tts_model_profile,
...
def build_translation_runtime_config(args) -> TranslationRuntimeConfig:
    payload = {
        ...
        "enable_local_llm_split": getattr(args, "enable_local_llm_split", False),
    }
    return TranslationRuntimeConfig.from_sources(payload)


def build_asr_runtime_config(args) -> AsrRuntimeConfig:
    service = getattr(args, "asr", None) or "faster-whisper"
    payload = {
        "asr_model_profile": getattr(args, "asr_model_profile", None),
        "faster_whisper_vad_filter": getattr(args, "faster_whisper_vad_filter", None),
        "faster_whisper_vad_threshold": getattr(args, "faster_whisper_vad_threshold", None),
        "funasr_batch_size_s": getattr(args, "funasr_batch_size_s", None),
        "funasr_merge_vad": getattr(args, "funasr_merge_vad", None),
        "local_asr_device": getattr(args, "local_asr_device", None),
        "local_asr_max_inference_batch_size": getattr(args, "local_asr_max_inference_batch_size", None),
        "local_asr_max_new_tokens": getattr(args, "local_asr_max_new_tokens", None),
        "qwen_device": getattr(args, "qwen_device", None),
        "qwen_max_inference_batch_size": getattr(args, "qwen_max_inference_batch_size", None),
        "qwen_max_new_tokens": getattr(args, "qwen_max_new_tokens", None),
    }
    return AsrRuntimeConfig.from_sources(service, payload)


def build_tts_runtime_config(args) -> TtsRuntimeConfig:
    payload = {
        "tts_service": getattr(args, "tts_service", None),
        "tts_model_profile": getattr(args, "tts_model_profile", None),
...
def build_dub_video_runtime_config(
    *,
    ...
    target_lang: str,
    output_path: str,
    asr_service: str = "faster-whisper",
    ...
    asr_payload = dict(kwargs)
    asr_payload["asr_model_profile"] = kwargs.get("asr_model_profile")
    asr = AsrRuntimeConfig.from_sources(asr_service, asr_payload)
    tts_payload = dict(kwargs)
    tts_payload["tts_service"] = tts_service
    tts = TtsRuntimeConfig.from_sources(tts_payload)
