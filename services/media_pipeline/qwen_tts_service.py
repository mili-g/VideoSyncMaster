...
def _setup_portable_audio_tools():
    ...
    candidate_bins = [
        get_media_tool_bin_dir(project_root, "ffmpeg"),
        get_media_tool_root(project_root, "sox"),
        os.path.join(get_media_tool_root(project_root, "sox"), "bin"),
    ]
...
_QWEN_TTS_PACKAGE_ROOT = os.path.join(_BACKEND_DIR, "Qwen3-TTS")
if os.path.isdir(_QWEN_TTS_PACKAGE_ROOT) and _QWEN_TTS_PACKAGE_ROOT not in sys.path:
    sys.path.insert(0, _QWEN_TTS_PACKAGE_ROOT)
...
try:
    ...
    Qwen3TTSModel = None
    QWEN_TTS_IMPORT_ERROR = str(error)
    print(f"[QwenTTS] Error: qwen-tts import failed: {error}")


def get_qwen_tts_runtime_status():
...
def _resolve_qwen_tts_model_size(kwargs):
    requested_size = str(kwargs.get("qwen_model_size", "") or "").strip()
    if requested_size in {"1.7B", "0.6B"}:
        return requested_size

    _, profile = get_tts_profile("qwen", kwargs.get("tts_model_profile"))
    return str(profile.get("qwen_model_size") or "1.7B")
...
def run_qwen_tts(text, ref_audio_path, output_path, language="Auto", **kwargs):
    ...
    model_size = _resolve_qwen_tts_model_size(kwargs)
