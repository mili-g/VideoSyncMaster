...
def _resolve_indextts_model_paths(model_dir=None, config_path=None, model_profile=None):
    ...
    profile_key, profile = get_tts_profile("indextts", model_profile)
    resolved_model_dir = model_dir or resolve_existing_path(profile.get("model_dir_candidates")) or DEFAULT_MODEL_DIR
    resolved_config_path = config_path or os.path.join(resolved_model_dir, "config.yaml")
    print(f"[IndexTTS] Using model profile: {profile_key}, model_dir={resolved_model_dir}")
    return resolved_model_dir, resolved_config_path
...
def run_tts(text, ref_audio_path, output_path, model_dir=None, config_path=None, language="English", **kwargs):
    ...
    model_dir, config_path = _resolve_indextts_model_paths(
        model_dir=model_dir,
        config_path=config_path,
        model_profile=kwargs.get("tts_model_profile"),
    )
    ...
    try:
        ...
        if not _should_keep_indextts_instance():
            _cleanup_indextts_instance()
