...
# services/media_pipeline/indextts/infer_v2.py
...
# HF_HOME must point to the parent root, not the hub directory itself.
# Pointing it to ".../hub" makes downstream resolution look for ".../hub/hub/...".
os.environ['HF_HOME'] = MODELS_ROOT
# Force local cache preference for vendored IndexTTS support models.
os.environ.setdefault('HF_HUB_OFFLINE', '1')
# And prevent offline issues if possible
# os.environ['HF_HUB_OFFLINE'] = "1" 

print(f"Set HF_HUB_CACHE to {HF_CACHE_DIR}")
...
from indextts.init_state import set_last_indextts_init_stage
from indextts.qwen_emotion import QwenEmotion
from indextts.similarity import find_most_similar_cosine
...
def _resolve_hf_snapshot_dir(repo_id):
    repo_folder = f"models--{repo_id.replace('/', '--')}"
    repo_root = os.path.join(HF_CACHE_DIR, repo_folder)
    ...
    if os.path.isfile(refs_main):
        try:
            with open(refs_main, "r", encoding="utf-8") as handle:
                snapshot_hash = handle.read().strip()
        except Exception:
            snapshot_hash = None
...
def _resolve_hf_model_source(repo_id):
    local_snapshot = _resolve_hf_snapshot_dir(repo_id)
    if local_snapshot and os.path.isdir(local_snapshot):
        print(f">> Using local HF snapshot for {repo_id}: {local_snapshot}")
        return local_snapshot
    print(f">> Local HF snapshot not found for {repo_id}, falling back to repo id.")
    return repo_id


def _hf_download_cached(repo_id, filename):
    local_snapshot = _resolve_hf_snapshot_dir(repo_id)
    if local_snapshot:
        candidate = os.path.join(local_snapshot, filename)
        if os.path.isfile(candidate):
            print(f">> Using cached HF file for {repo_id}: {candidate}")
            return candidate
...
class IndexTTS2:
    def __init__(
        ...
        """
        self.device, self.use_fp16, self.use_cuda_kernel = self._resolve_runtime_device(
            device=device,
            use_fp16=use_fp16,
        ...
        self.qwen_emo = self._init_qwen_emotion()
        self.gpt = self._init_gpt(use_deepspeed=use_deepspeed)
        self._preload_bigvgan_cuda_kernel()
        self._init_semantic_stack()
        self.s2mel = self._init_s2mel()
        self.campplus_model = self._init_campplus()
        self.bigvgan = self._init_bigvgan()
        self.normalizer, self.tokenizer = self._init_text_frontend()
        self.emo_matrix, self.spk_matrix, self.emo_num = self._init_emotion_and_speaker_matrices()
        self.mel_fn = self._build_mel_fn()
    ...
    def _set_init_stage(self, stage_name):
        ...
        set_last_indextts_init_stage(stage_name)
        print(f">> IndexTTS init stage: {stage_name}")

    def _resolve_runtime_device(self, device, use_fp16, use_cuda_kernel):
        if device is not None:
            resolved_device = device
            resolved_fp16 = False if device == "cpu" else use_fp16
            resolved_cuda_kernel = bool(use_cuda_kernel) and device.startswith("cuda")
        elif torch.cuda.is_available():
            resolved_device = "cuda:0"
            resolved_fp16 = use_fp16
            resolved_cuda_kernel = use_cuda_kernel is None or use_cuda_kernel
        elif hasattr(torch, "xpu") and torch.xpu.is_available():
            resolved_device = "xpu"
            resolved_fp16 = use_fp16
            resolved_cuda_kernel = False
        ...
        return resolved_device, resolved_fp16, resolved_cuda_kernel

    def _init_qwen_emotion(self):
        ...
        return QwenEmotion(qwen_path)

    def _resolve_deepspeed_flag(self, use_deepspeed):
        if not use_deepspeed:
            return False

        try:
            import deepspeed  # noqa: F401
        except (ImportError, OSError, CalledProcessError) as exc:
            print(f">> Failed to load DeepSpeed. Falling back to normal inference. Error: {exc}")
            return False
        return True

    def _init_gpt(self, use_deepspeed):
        self._set_init_stage("gpt_build")
        gpt = UnifiedVoice(**self.cfg.gpt, use_accel=self.use_accel)
        self.gpt_path = os.path.join(self.model_dir, self.cfg.gpt_checkpoint)
        self._set_init_stage("gpt_checkpoint")
        load_checkpoint(gpt, self.gpt_path)
        ...
        gpt.post_init_gpt2_config(
            use_deepspeed=self._resolve_deepspeed_flag(use_deepspeed),
            kv_cache=True,
            half=self.use_fp16,
        )
        return gpt

    def _preload_bigvgan_cuda_kernel(self):
    ...
    def _init_semantic_stack(self):
        self._set_init_stage("feature_extractor")
        self.extract_features = SeamlessM4TFeatureExtractor.from_pretrained(
            _resolve_hf_model_source("facebook/w2v-bert-2.0"),
        ...
        self.semantic_model = semantic_model.to(self.device)
        ...
        semantic_code_ckpt = _hf_download_cached("amphion/MaskGCT", filename="semantic_codec/model.safetensors")
    ...
    def _init_campplus(self):
        campplus_ckpt_path = _hf_download_cached("funasr/campplus", filename="campplus_cn_common.bin")
    ...
    def _init_bigvgan(self):
        ...
        vocoder = bigvgan.BigVGAN.from_pretrained(bigvgan_name, use_cuda_kernel=self.use_cuda_kernel)
    ...
    def _init_emotion_and_speaker_matrices(self):
        ...
        return torch.split(emo_matrix, emo_num), torch.split(spk_matrix, emo_num), emo_num
    ...
    def infer_generator(self, spk_audio_prompt, text, output_path,
        ...
        print(f">> GPT前向传播时间: {gpt_forward_time:.2f} 秒")
        print(f">> S2Mel生成时间: {s2mel_time:.2f} 秒")
        print(f">> BigVGAN生成时间: {bigvgan_time:.2f} 秒")
        print(f">> 总推理时间: {end_time - start_time:.2f} 秒")
        print(f">> 生成音频时长: {wav_length:.2f} 秒")
        print(f">> 实时率(RTF): {(end_time - start_time) / wav_length:.4f}")
