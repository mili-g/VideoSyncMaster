...
import transformers
from transformers import GPT2Config, LogitsProcessorList, GenerationMixin
...
def _normalize_generation_config_compat(generation_config):
    if generation_config is None:
        return None

    # Vendored generation utils assume a superset of GenerationConfig fields
    # across multiple Transformers releases. Normalize missing optional fields
    # once at model init instead of scattering getattr guards throughout the stack.
    ...
    for attr, default_value in compatibility_defaults.items():
        if not hasattr(generation_config, attr):
            setattr(generation_config, attr, default_value)
...
class UnifiedVoice(nn.Module):
    ...
    def post_init_gpt2_config(self, use_deepspeed=False, kv_cache=False, half=False):
        ...
        self.inference_model.generation_config = _normalize_generation_config_compat(
            getattr(self.inference_model, "generation_config", None)
        )
