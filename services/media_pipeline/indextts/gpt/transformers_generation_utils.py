...
try:
    ...
    class QuantizedCacheConfig:  # pragma: no cover - compatibility shim for local vendored runtime
...
try:
    ...
    def _crop_past_key_values(_model, past_key_values, max_length):  # pragma: no cover - compatibility shim
        if past_key_values is None:
            return past_key_values
        if hasattr(past_key_values, "crop"):
            cropped = past_key_values.crop(max_length)
            return past_key_values if cropped is None else cropped
            ...
            for layer in past_key_values:
                if isinstance(layer, tuple):
                    cropped_layers.append(
                        tuple(
                            tensor[..., :max_length, :] if hasattr(tensor, "shape") and tensor.ndim >= 3 else tensor
                            for tensor in layer
                        )
                    )
                else:
