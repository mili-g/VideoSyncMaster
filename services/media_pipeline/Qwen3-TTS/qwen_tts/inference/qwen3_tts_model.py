        ...
        **kwargs,
        ...
        for i, codes in enumerate(talker_codes_list):
            ...
            if ref_code_list is not None and ref_code_list[i] is not None:
                ref_c = ref_code_list[i].to(codes.device)
                # Check if codes already starts with ref_c to avoid duplication
                if codes.shape[0] >= ref_c.shape[0] and torch.equal(codes[:ref_c.shape[0]], ref_c):
                     print(f"[Qwen3TTSModel] Generated codes already contain reference prefix (len={ref_c.shape[0]}). Skipping concat.", flush=True)
                     codes_for_decode.append(codes)
                else:
                     # print(f"[Qwen3TTSModel] Concatenating reference prefix (len={ref_c.shape[0]}).", flush=True)
        ...
        for i, wav in enumerate(wavs_all):
            ...
            if ref_code_list is not None and ref_code_list[i] is not None:
                ...
                print(f"[Qwen3TTSModel] Slicing: ref_len={ref_len}, total_len={total_len}, wav_len={wav.shape[0]}, cut={cut}", flush=True)
                wavs_out.append(wav[cut:])
            else:
                print(f"[Qwen3TTSModel] No slicing (ref_code_list missing or None for item {i})", flush=True)
                wavs_out.append(wav)
