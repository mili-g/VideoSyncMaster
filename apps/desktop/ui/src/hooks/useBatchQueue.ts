...
import { cleanupOutputArtifacts } from '../utils/outputArtifacts';
...
function readStoredQueueItems(): BatchQueueItem[] {
    try {
        ...
        return parsed.map((item) => {
            const rest = item && typeof item === 'object'
...
export function useBatchQueue(_options: UseBatchQueueOptions = {}) {
    ...
    const resetQueueRuntimeState = (timestamp = Date.now()) => {
        ...
        try {
            localStorage.setItem(BATCH_QUEUE_META_STORAGE_KEY, JSON.stringify(persistedMetaRef.current));
        } catch (error) {
            console.error('Failed to reset batch queue meta:', error);
        }
    };
        ...
        while (
            ...
            void window.api.analyzeVideoMetadata(task.path)
                .then(result => {
                    ...
                    if (result?.success && typeof duration === 'number') {
                        updateItem(task.id, current => ({
                            ...current,
                            sourceDurationSec: Number(duration) || 0
                        }));
                    }
                })
                .catch(error => {
    ...
    const addAssets = async (assets: BatchInputAsset[]) => {
        ...
        for (const asset of subtitleAssets) {
            ...
            if (!matchedItem || classifyBatchAsset(asset) === 'unknown') {
                unresolved.push({
                    ...asset,
                    suggestedItemId: matchedItem?.id,
                    requiresManualAssignment: classifyBatchAsset(asset) === 'unknown'
                });
                continue;
            }
    ...
    const cleanupQueueItemsArtifacts = async (queueItems: BatchQueueItem[]) => {
        if (queueItems.length === 0) return;
        const paths = await window.api.getPaths();
        const cleanupTargets = new Set<string>();

        for (const item of queueItems) {
            const projectPaths = buildBatchOutputPaths(
                paths,
                item.fileName,
                item.id,
                resolveItemOutputDir(item, _options.outputDirOverride)
            );
            cleanupTargets.add(projectPaths.sessionCacheDir);
            if (item.status !== 'success') {
                cleanupTargets.add(projectPaths.finalDir);
            }
        }

        await cleanupOutputArtifacts('', Array.from(cleanupTargets));
    };

    const removeItem = async (id: string) => {
        durationProbeQueueRef.current = durationProbeQueueRef.current.filter(task => task.id !== id);
        const targetItem = itemsRef.current.find(item => item.id === id && item.status !== 'processing');
        if (targetItem) {
            await cleanupQueueItemsArtifacts([targetItem]);
        }
        setItems(prev => prev.filter(item => item.id !== id || item.status === 'processing'));
    };

    const clearCompleted = async () => {
        const removableItems = itemsRef.current.filter(item => item.status !== 'pending' && item.status !== 'processing');
        await cleanupQueueItemsArtifacts(removableItems);
    ...
    const clearAll = async () => {
        ...
        await cleanupQueueItemsArtifacts(itemsRef.current);
        pendingSubtitleAssetsRef.current = [];
        setUnmatchedSubtitleAssets([]);
        durationProbeQueueRef.current = [];
        durationProbeActiveCountRef.current = 0;
        clearDurationProbeTimer();
        setItems([]);
        itemsRef.current = [];
        try {
            localStorage.setItem(BATCH_QUEUE_ITEMS_STORAGE_KEY, JSON.stringify([]));
        } catch (error) {
            console.error('Failed to reset batch queue items:', error);
        }
        resetQueueRuntimeState(clearedAt);
    };
    ...
    useEffect(() => {
        ...
        if (missingDurationItems.length > 0) {
            const queuedIds = new Set(durationProbeQueueRef.current.map(task => task.id));
            for (const task of missingDurationItems) {
                if (queuedIds.has(task.id) || !isDurationProbeEligible(task)) continue;
                durationProbeQueueRef.current.push(task);
                queuedIds.add(task.id);
            }
