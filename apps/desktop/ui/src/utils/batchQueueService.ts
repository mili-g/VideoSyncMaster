...
import { cleanupOutputArtifacts, saveOriginalSubtitleArtifact, saveSubtitleArtifacts } from './outputArtifacts';
import { prepareBatchProjectPaths } from './projectPaths';
import { appendStoredAsrArgs, appendStoredTranslationArgs, getStoredTtsVoiceMode } from './runtimeSettings';
import { getStoredAsrModelProfile } from './modelProfiles';
...
import { buildMergeVideoCommand, buildTestAsrCommand, buildTranslateTextCommand } from './backendCommandBuilders';
...
import { getAsrSourceLanguageConstraint, resolveEffectiveAsrSourceLanguage, type AsrService } from './asrService';
import { resolveSubtitleArtifactLanguages } from './languageTags';
import { validateSubtitleLanguageFit } from './batchAssets';
import { assertTranslatedSegmentsDifferFromSource } from './translationIntegrity';
import {
    BATCH_QUEUE_STAGE,
    createStage,
...
export async function generateSubtitleForItem(
    ...
    if (sourceLanguageConstraint) {
        throw new Error(sourceLanguageConstraint);
    }
    const command = buildTestAsrCommand({
        input: item.sourcePath,
        asrService: options.asrService,
        asrModelProfile: getStoredAsrModelProfile(asrService),
        outputDir: projectPaths.sessionTempDir,
        sourceLanguage: resolveEffectiveAsrSourceLanguage(asrService, options.asrOriLang),
        json: true
    });
...
export async function prepareQueueItem(
    ...
    try {
        ...
        assertSubtitleLanguageFit(
            buildSubtitleValidationText(sourceSegments, sourceSubtitleContent),
            options.asrOriLang,
            'source'
        );
        ...
        assertSubtitleLanguageFit(
            buildSubtitleValidationText(translatedSegments, translatedSubtitleContent),
            options.targetLang,
            'target'
        );
...
export async function finalizeQueueItem(
    ...
    try {
        ...
        if (pendingSegments.length > 0) {
            ...
            const ttsResult = await runBackendCommand(
                buildBatchTtsArgs(
                    item.sourcePath,
                    projectPaths.sessionAudioDir,
                    tempJsonPath,
        ...
        if (failedIndexes.length > 0) {
            ...
            await retryFailedBatchSegments({
                sourcePath: item.sourcePath,
                sessionAudioDir: projectPaths.sessionAudioDir,
                workDir: projectPaths.sessionTempDir,
                sourceSegments,
        ...
        const mergeResult = await runBackendCommand(buildMergeVideoCommand({
            input: item.sourcePath,
            output: outputPath,
            ref: mergeJsonPath,
...
export async function translateSegments(
    ...
    const translatedSegments = result.segments as SrtSegment[];
    assertTranslatedSegmentsDifferFromSource(segments, translatedSegments);
        ...
        try {
            ...
            onItemUpdate(item.id, current => ({
                ...
                ...createStage(
                    ...
                    item.originalSubtitleContent ? '原字幕已刷新，原有译文已清理，请重新翻译' : '原字幕已生成，可继续翻译'
                ),
                resolvedOutputDir: outputDir,
                originalSubtitlePath: subtitlePath,
                originalSubtitleContent: subtitleContent,
                translatedSubtitlePath: undefined,
                translatedSubtitleContent: undefined,
                errorInfo: undefined,
                finishedAt: Date.now(),
    ...
    for (const item of queue) {
        ...
        try {
            const { projectPaths } = await prepareBatchProjectPaths(
                ...
                item.id,
                resolveItemOutputDir(item, options.outputDirOverride),
                resolveSubtitleArtifactLanguages(undefined, options.targetLang)
            );
            ...
            assertSubtitleLanguageFit(
                buildSubtitleValidationText(sourceSegments, item.originalSubtitleContent),
                options.asrOriLang,
                'source'
            );
...
function buildSubtitleValidationText(segments: SrtSegment[], rawContent?: string) {
    const segmentText = segments
        .map(segment => String(segment.text || '').trim())
        .filter(Boolean)
        .join(' ');

    return segmentText || String(rawContent || '');
}

function assertSubtitleLanguageFit(
    text: string,
    ...
    if (!result.ok) {
        throw new Error(result.reason || '字幕语言与当前配置不匹配。');
    }
}
...
async function retryFailedBatchSegments({
    ...
    for (const index of failedIndexes) {
        ...
        const outputPath = `${sessionAudioDir}\\segment_retry_${index}.wav`;
