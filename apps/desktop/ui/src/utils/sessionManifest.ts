...
export async function getSessionResumePlan(
    ...
    const [existingSourceSubtitles, existingTranslatedSubtitles, manifestPreservedSegments, liveAudioSegments] = await Promise.all([
        ...
        listResumeAudioSegments(target.sessionAudioDir)
    ]);
    const preservedAudioSegments = Array.from(new Set([
        ...manifestPreservedSegments,
        ...liveAudioSegments
    ]));
