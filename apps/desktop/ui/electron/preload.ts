...
contextBridge.exposeInMainWorld('api', {
  ...
  readBackendLog() {
    return ipcRenderer.invoke('read-backend-log')
  },
  clearBackendLog() {
    return ipcRenderer.invoke('clear-backend-log')
  },
  installFunasrRuntime(payload: { key: string }) {
    return ipcRenderer.invoke('install-funasr-runtime', payload)
  },
  killBackend() {
  ...
  installTransformers5AsrRuntime(payload: { key: string }) {
    return ipcRenderer.invoke('install-transformers5-asr-runtime', payload)
  },
  ...
  onModelDownloadProgress(listener: (data: unknown) => void) {
    const wrapped = (_event: unknown, data: unknown) => listener(data)
    ipcRenderer.on('model-download-progress', wrapped)
    return () => ipcRenderer.off('model-download-progress', wrapped)
  },
